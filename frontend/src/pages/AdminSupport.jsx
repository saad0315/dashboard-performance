...
import { getAdminChatrooms } from '../api/chatsApi';

const AdminSupport = () => {
    // const socket = useContext(SocketContext);
    const { socket, onlineUsers } = useSocket(); // Use socket from context
    ...
    const {
        ...
        queryFn: getAdminChatrooms,  // Fetches users with last message and unseen count
    });
    ...
    useEffect(() => {
        const sortedUsers = [...(usersWithLastMessage?.chatUsers || [])]
            .sort((a, b) => new Date(b.lastMessage?.createdAt) - new Date(a.lastMessage?.createdAt))
            .filter((d) => {
                const user1Name = d?.user1?.userName?.toLowerCase() || "";
                const user2Name = d?.user2?.userName?.toLowerCase() || "";
                const search = searchUser.toLowerCase();

                // Fix: Use OR condition to match either user1 or user2
    ...
    const selectUser = (user) => {
        ...
        // apiMain.post(`/mark-seen/${user._id}`).catch((err) => console.log(err)); // Mark messages as seen
        setMessages([]);
    ...
    const getMessages = async (userId1, userId2) => {
        try {
            const res = await apiMain.get(`/admin/userchats/${userId1}/${userId2}`);
    ...
    useEffect(() => {
        if (selectedUser) {
            ...
            getMessages(selectedUser?.user1?._id, selectedUser?.user2?._id);
        }
    }, [selectedUser]);
    ...
    //         });
    //         try {
    //             const res = await apiMain.post('/newsend', message);
    //             setMessages([...messages, res?.data?.newMessage]);
    //             console.log("sendmessage", res.data?.newMessage);
    //         } catch (err) {
    //             console.log(err);
    //         }
    ...
    const sendMessage = async () => {
        if (textMessage.trim()) {
            ...
            try {
                ...
                // Update sidebar user list (move sender to top)
                setFilteredItems((prevUsers) => {
                    return prevUsers.map((u) =>
                        u._id === selectedUser._id
                            ? { ...u, lastMessage: newMessage } // Update last message
                            : u
                    ).sort((a, b) => new Date(b.lastMessage?.createdAt) - new Date(a.lastMessage?.createdAt));
                });
                                ...
                                ?.map((person, index) => {
                                    ...
                                    return (
                                        <div key={index}>
                                            <button
                                                type="button"
                                                className={`w-full flex justify-between items-center p-2 hover:bg-gray-100 dark:hover:bg-[#050b14] rounded-md dark:hover:text-primary hover:text-primary ${selectedUser && selectedUser?.user1?._id === person?.user1?._id && selectedUser?.user2?._id === person?.user2?._id ? 'bg-gray-100 dark:bg-[#050b14] dark:text-primary text-primary' : ''
                                                    }`}
                                                onClick={() => selectUser(person)}
                                            >
                                                <div className="flex-1">
                                                    <div className={`flex items-center relative ${unseenMessages ? '' : ''}`}>
                                                        ...
                                                        <div className="mx-3 ltr:text-left rtl:text-right ">
                                                            <p className="mb-1 font-semibold capitalize">{person?.user1?.userName} , {person?.user2?.userName}</p>
                                                            <p className="text-xs text-white-dark truncate max-w-[185px]">
                                                                {person?.user1?.role} , {person?.user2?.role}
                                                            </p>
                                                        </div>
                                                    </div>
                                                </div>
                        ...
                        <div className="relative h-full">
                            <div className="flex justify-between items-center p-4">
                                <div className="flex items-center space-x-2 rtl:space-x-reverse">
                                    ...
                                    <div className="mx-3">
                                        <p className="font-semibold capitalize">{selectedUser?.user1?.userName} , {selectedUser?.user2?.userName}</p>
                                        <p className="text-xs text-white-dark truncate ">
                                            {selectedUser?.user1?.userEmail} , {selectedUser?.user2?.userEmail}
                                        </p>
                            ...
                            <PerfectScrollbar className="relative h-full sm:h-[calc(100vh_-_300px)] chat-conversation-box" containerRef={ref => chatContainerRef.current = ref} >
                                            ...
                                            {messages?.map((message, index) => {
                                                return (
                                                    <div key={index}>
                                                        ...
                                                        <div className={`flex items-start gap-3 ${selectedUser?.user1?._id !== message.senderId ? 'justify-end' : ''}`}>
                                                            <div className={`flex-none ${selectedUser?.user1?._id !== message.senderId ? 'order-2' : ''}`}>
                                                                {selectedUser?.user1?._id === message.receiverId ? (
                                                            ...
                                                            <div className="space-y-2">
                                                                <div className="flex items-center gap-3">
                                                                    <div
                                                                        className={`dark:bg-gray-800 p-4 py-2 rounded-md bg-black/10 ${selectedUser?.user1?._id !== message.receiverId
                                                                    ...
                                                                    <div className={`${message.receiverId !== message.senderId ? 'hidden' : ''}`}>
                                                                        <IconMoodSmile className="hover:text-primary" />
                                                                    </div>
                                                                </div>
                                                                <div className={`text-xs text-white-dark ${message.receiverId !== message.senderId ? 'ltr:text-right rtl:text-left' : ''}`}>
