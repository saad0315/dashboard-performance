...
import { detectAndConvertLinks } from '../constants/constants';
import { getChatrooms } from '../api/chatsApi';
import { Dialog } from '@headlessui/react';
import { formatFileSize, getReadableFileType } from '../utils/Utils';
...
const Support = () => {
    ...
    // Sorting users by last message date and filtering based on search
    useEffect(() => {
        const sortedUsers = [...(usersWithLastMessage?.users || [])]
            .sort((a, b) => new Date(b.lastMessage?.createdAt) - new Date(a.lastMessage?.createdAt))
            .filter((d) => d?.userName.toLowerCase().includes(searchUser.toLowerCase()) && d?._id !== user?._id);

        setFilteredItems(sortedUsers);
    }, [searchUser, usersWithLastMessage, arrivalMessages]);
    ...
    // Select a user for chat and mark messages as seen
    const selectUser = (user) => {
        setSelectedUser(user);
        setIsShowUserChat(true);
    ...
    const getMessages = async (userId) => {
        try {
            const res = await apiMain.get(`/newmessages/${userId}`);
    ...
    useEffect(() => {
        const handleGetMessage = async (data) => {
            ...
            const newMessage = {
                ...
                createdAt: Date.now(),
                seen: selectedUser?._id === data.senderId, // Agar user chat ma hai to seen mark karo
            };

            setArrivalMessages(newMessage);

            if (selectedUser?._id === data.senderId) {
                try {
                    await apiMain.post("/mark-message-seen", { messageId: data._id });
                } catch (error) {
        ...
        if (socket) {
            ...
            return () => socket.off('receiveMessage', handleGetMessage);
        }
    }, [socket, selectedUser]);

    useEffect(() => {
        if (arrivalMessages && selectedUser?._id === arrivalMessages.senderId) {
            setMessages((prevMessages) => [...prevMessages, arrivalMessages]);
        }
    }, [arrivalMessages, selectedUser]);
    ...
    const sendMessage = async () => {
        if (textMessage.trim() || selectedFile) {
            ...
            //     senderId: user?._id,
            //     createdAt: new Date(), // Add timestamp immediately
            // };
            ...
            if (selectedFile) {
                formData.append("file", selectedFile); // this will go to multerS3
            }
            try {
                const res = await apiMain.post('/newsend', formData, {
                    headers: {
    ...
    return (
        <div>
            <div className={`flex gap-5 relative h-[calc(100vh_-_150px)]   ${isShowChatMenu ? 'min-h-[999px]' : ''} `}>
                <div className={`panel p-4 flex-none max-w-xs w-full absolute xl:relative z-10 space-y-4 xl:h-full hidden xl:block overflow-hidden ${isShowChatMenu ? '!block' : ''}`}>
                    <div className="flex justify-between items-center">
                        <div className="flex items-center">
                            <div className="flex-none">
                            ...
                            <div className="mx-3">
                                ...
                                <p className="text-xs text-white-dark">{user.userEmail}</p>
                            </div>
                        </div>
                        {/* <div className="dropdown">
                            <Dropdown
                    ...
                    <div className="!mt-0">
                        {/* <PerfectScrollbar className="chat-users relative h-full min-h-[100px] sm:h-[calc(100vh_-_357px)] space-y-0.5 ltr:pr-3.5 rtl:pl-3.5 ltr:-mr-3.5 rtl:-ml-3.5">
                            {filteredItems
                                ?.sort((a, b) => new Date(b?.lastMessage?.createdAt) - new Date(a?.lastMessage?.createdAt))
                                .map((person, index) => {
                                    const lastMessage = person?.lastMessage;
                                    // Find the user (either customer or representative) based on your logic
                                    const username = person?.members?.find(member => member._id !== user._id); // Excluding the logged-in user
                                    console.log("rank", lastMessage);
                                    return (
                                        <div key={index}>
                                            <button
                                                type="button"
                                                className={`w-full flex justify-between items-center p-2 hover:bg-gray-100 dark:hover:bg-[#050b14] rounded-md dark:hover:text-primary hover:text-primary ${selectedUser && selectedUser._id === person._id ? 'bg-gray-100 dark:bg-[#050b14] dark:text-primary text-primary' : ''
                                                    }`}
                                                onClick={() => selectUser(person)}
                                            >
                                                <div className="flex-1">
                                                    <div className="flex items-center">
                                                        ...
                                                        <div className="mx-3 ltr:text-left rtl:text-right">
                                                            <p className="mb-1 font-semibold">{username?.userName}</p>
                                                            <p className="text-xs text-white-dark truncate max-w-[185px]">
                                                                {person?.lastMessage?.sender === user.id ? `You : ${person?.lastMessage?.text}` : person.lastMessage?.text}
                                                            </p>
                                                        </div>
                                                    </div>
                                                </div>
                        ...
                        <PerfectScrollbar className="chat-users relative min-h-[100px] h-[calc(100vh_-_357px)] space-y-0.5 ltr:pr-3.5 rtl:pl-3.5 ltr:-mr-3.5 rtl:-ml-3.5">
                            ...
                            {filteredItems
                                ?.map((person, index) => {
                                    const lastMessage = person?.lastMessage;
                                    // Find the user (either customer or representative) based on your logic
                                    const unseenMessages = person?.unseenMessagesCount; // Excluding the logged-in user
                                    const username = person?.members?.find(member => member._id !== user._id); // Excluding the logged-in user
                                    ...
                                    return (
                                        <div key={index}>
                                            ...
                                            >
                                                <div className="flex-1">
                                                    <div className={`flex items-center relative ${unseenMessages ? 'text-primary' : ''}`}>
                                                        <div className="flex-shrink-0 relative ">
                                                            <img
                                                        ...
                                                        <div className="mx-3 ltr:text-left rtl:text-right ">
                                                            <p className="mb-1 font-semibold">{person?.userName}</p>
                                                            {/* <p className="text-xs text-white-dark truncate max-w-[185px]">
                                                                {person?.lastMessage?.sender === user.id ? `You : ${person?.lastMessage?.text}` : person.lastMessage?.text}
                                                            </p> */}
                                                            ...
                                                            {unseenMessages > 0 && (
                                                                <div>
                                                                    <div className="absolute bottom-0 ltr:right-0 rtl:left-0">
                                                                        <div className="w-4 h-4 text-primary rounded-full ">{unseenMessages}</div>
                                                                    </div>
                                                                </div>
                                                            )}
                                                        </div>
                                                ...
                                                <div className="font-semibold whitespace-nowrap text-xs">
                                                    <p>{person?.lastMessage ? new Date(person?.lastMessage?.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : ''}</p>
                                                </div>
                                            </button>
                                        </div>
                                    );
                ...
                <div className="panel p-0 flex-1 ">
                    ...
                    {isShowUserChat && selectedUser ? (
                        <div className="relative h-full">
                            <div className="flex justify-between items-center p-4">
                                <div className="flex items-center space-x-2 rtl:space-x-reverse">
                                    ...
                                    <div className="mx-3">
                                        ...
                                        <p className="text-xs text-white-dark truncate ">
                                            {selectedUser?.userEmail}
                                        </p>
                                        {/* <p className="font-semibold">
                                            {selectedUser?.members?.find(member => member._id !== user.id)?.userName}
                                        </p> */}
                                        {/* <p className="text-white-dark text-xs">{selectedUser.active ? 'Active now' : 'Last seen at ' + selectedUser.time}</p> */}
                                    </div>
                                </div>
                            ...
                            <PerfectScrollbar className="relative h-[calc(100vh_-_320px)] chat-conversation-box flex flex-col justify-between " containerRef={ref => chatContainerRef.current = ref} >
                                <div className="space-y-5 p-4 sm:pb-0 pb-[68px] sm:min-h-[300px] min-h-[400px]">
                                    {/* <div className="block m-6 mt-0">
                                        <h4 className="text-xs text-center border-b border-[#f4f4f4] dark:border-gray-800 relative">
                                            <span className="relative top-2 px-3 bg-white dark:bg-black">{'Today, ' + selectedUser.time}</span>
                                            ...
                                            {messages?.map((message, index) => {
                                                ...
                                                return (
                                                    <div key={index}>
                                                        ...
                                                        <div className={`flex items-start gap-3 ${user?._id === message.senderId ? 'justify-end' : ''}`}>
                                                            <div className={`flex-none ${user._id === message.senderId ? 'order-2' : ''}`}>
                                                                {selectedUser._id === message.receiverId ? (
                                                                    <img src={`https://res.cloudinary.com/ddvtgfqgv/image/upload/v1691059545/member-profile/avatar_aoyxl6.webp`}
                                                                        className="rounded-full h-10 w-10 object-cover" alt="" />
                                                                    // <img src={`/assets/images/${loginUser.path}`} className="rounded-full h-10 w-10 object-cover" alt="" />
                                                                ) : (
                                                                    ''
                                                                )}
                                                                {selectedUser.userId === message.receiverId ? (
                                                                    <img
                                                                        src={`https://res.cloudinary.com/ddvtgfqgv/image/upload/v1691059545/member-profile/avatar_aoyxl6.webp`}
                                                                        className="rounded-full h-10 w-10 object-cover" alt="" />
                                                                ) : (
                                                            ...
                                                            <div className="space-y-2">
                                                                <div className="flex flex-col  gap-3 sm:gap-4 max-w-full break-words">
                                                                    {message.text?.trim() && (
                                                                        <div
                                                                            className={`dark:bg-gray-800 p-4 py-2 rounded-md bg-black/10 w-fit max-w-[90%] sm:max-w-[70%] break-words ${message.senderId === user._id
                                                                                ? 'ltr:rounded-br-none rtl:rounded-bl-none !bg-primary text-white ml-auto'
                                                                                : 'ltr:rounded-bl-none rtl:rounded-br-none'
                                                                                }`}
                                                                        >
                                                                            {detectAndConvertLinks(message.text)}
                                                                        </div>
                                                                    )}
                                                                    {fileData && (
                                                                        <div className="mt-2 sm:mt-0 max-w-xs w-full">
                                                                            {isImage ? (
                                                                                <>
                                                                                    <img
                                                                                        src={`https://ebook-sales.b-cdn.net/ebook-sales/${fileData.src}`}
                                                                                        alt={fileData.name}
                                                                                        className="rounded cursor-pointer object-cover w-full h-auto"
                                                                                        onClick={() => setOpenImage(fileData)}
                                                                                    />
                                                                                    <a
                                                                                        href={`https://ebook-sales.b-cdn.net/ebook-sales/${fileData.src}`}
                                                                                        download={fileData.name}
                                                                                        target='_blank'
                                                                                        className="flex items-center gap-2 text-sm text-blue-600 mt-1 hover:underline"
                                                                                    >
                                                                                    ...
                                                                                    {
                                                                                        openImage && (
                                                                                            <Dialog open={true} onClose={() => setOpenImage(null)} className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
                                                                                                <Dialog.Panel className="max-w-3xl w-full p-4 bg-white rounded shadow-lg">
                                                                                                    <img
                                                                                                        src={`https://ebook-sales.b-cdn.net/ebook-sales/${openImage?.src}`}
                                                                                                        alt={openImage?.name}
                                                                                ...
                                                                                //     src={`https://ebook-sales.b-cdn.net/ebook-sales/${fileData.src}`}
                                                                                //     alt={fileData.name}
                                                                                //     className=" rounded h-auto max-w-full object-contain"
                                                                                // />
                                                                            ) : (
                                                                                // <a
                                                                                //     href={`https://ebook-sales.b-cdn.net/ebook-sales/${fileData.src}`}
                                                                                ...
                                                                                //     {fileData.name}
                                                                                // </a>
                                                                                <div className="flex items-center justify-between bg-gray-100 border rounded-md p-4 w-full">
                                                                                    <div className="flex flex-col max-w-[80%]">
                                                                                        <span className="font-medium text-sm truncate">{fileData.name}</span>
                                                                                        <span className="text-xs text-gray-600 mt-1">
                                                                                            {getReadableFileType(fileData.type) || 'File'} • {formatFileSize(fileData.size)}
                                                                                        </span>
                                                                                    </div>
                                                                                    <a
                                                                                        href={`https://ebook-sales.b-cdn.net/ebook-sales/${fileData.src}`}
                                                                    ...
                                                                    <div className={`${user._id === message.senderId ? 'hidden' : ''}`}>
                                                                        <IconMoodSmile className="hover:text-primary" />
                                                                    </div>
                                                                </div>
                                                                <div className={`text-xs text-white-dark ${user?._id === message.senderId ? 'ltr:text-right rtl:text-left' : ''}`}>
                                                                    {formatDistanceToNow(new Date(message.createdAt ? message.createdAt : Date.now()), { addSuffix: true })}
                                                                    {/* {message.createdAt ? new Date(message.createdAt).toLocaleDateString() : new Date().toLocaleDateString()} */}
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                ...
                                {selectedFile && (
                                    <div className="flex flex-col sm:flex-row items-end justify-center gap-3 pt-4  px-2 sm:px-0 self-end ">
                                        {selectedFile.type.startsWith('image') && (
                                            <img
                                                src={URL.createObjectURL(selectedFile)}
                                                alt="Preview"
                                                className="max-h-30 max-w-xs rounded-md object-contain"
                                            />
                                        )}

                                        <div className="flex items-center space-x-2 bg-gray-200 dark:bg-gray-700 px-3 py-1 rounded-full max-w-full sm:max-w-xs overflow-hidden">
                            ...
                            <div className="p-4 absolute bottom-0 left-0 w-full">
                                <div className="flex flex-wrap sm:flex-nowrap items-center gap-3 w-full">
                                    <div className="relative flex-1 w-full">
                                        <input
                                            className="form-input w-full rounded-full border-0 bg-[#f4f4f4] px-12 focus:outline-none "
                                            placeholder="Type a message"
                                            value={textMessage}
                                            onChange={(e) => setTextMessage(e.target.value)}
                                            onKeyUp={sendMessageHandle}
                                        ...
                                        <div className="flex justify-center items-center gap-2 absolute ltr:right-4 rtl:left-4 top-1/2 -translate-y-1/2">
                                            ...
                                            <input
                                                ...
                                                onPaste={handlePaste}
                                            />
                                            <label htmlFor="fileInput" className="bg-[#f4f4f4] dark:bg-[#1b2e4b] hover:text-primary rounded-md p-2 cursor-pointer">
                                                <IconDownload />
                                            </label>
                                        </div>
                                    </div>
