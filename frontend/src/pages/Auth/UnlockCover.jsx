...
const UnlockCover = () => {
    ...
    const desiredPath = pathParts.slice(0, 3).join('/');
    ...
    const mutation = useMutation({
        ...
        mutationFn: (data) => createPassword(data, token),
        onSuccess: (response) => {
          
            dispatch(setUser(response?.data));
            localStorage.setItem('user', JSON.stringify(response?.data?.user));
            localStorage.setItem('token', response?.data?.token);
            coloredToast('success', `Welcome Back, ${capitalizeTitle(response?.data?.user?.userName)}`, 'top');
            navigate('/');
        },
        onError: (error) => {
            console.log(error);
            ...
            <div className="relative flex min-h-screen items-center justify-center bg-[url(/assets/images/auth/map.png)] bg-cover bg-center bg-no-repeat px-6 py-10 dark:bg-[#060818] sm:px-16">
                ...
                <div className="relative flex w-full max-w-[1502px] flex-col justify-between overflow-hidden rounded-md bg-white/60 backdrop-blur-lg dark:bg-black/50 lg:min-h-[758px] lg:flex-row lg:gap-10 xl:gap-0">
                    ...
                    <div className="relative flex w-full flex-col items-center justify-center gap-6 px-4 pb-16 pt-6 sm:px-6 lg:max-w-[667px]">
                        ...
                        <div className="w-full max-w-[440px] lg:mt-16">
                            <div className="mb-10 flex items-center">
                                ...
                                <div className="flex-1">
                                    <h4 className="text-2xl dark:text-white">Hey User,</h4>
                                    {desiredPath && desiredPath === '/password/create' ? (
                                        <p className="text-gray-400 text-xs capitalize">You are setting your password for the first time by link sent to your email.</p>
                                    ) : (
                                        <p className="text-gray-400 text-xs capitalize">create a new Password</p>
                                    )}
                                    {/* <p className="text-white-dark">Enter your password to unlock your ID</p> */}
                                </div>
                            </div>
                            <Formik initialValues={{ password: '', confirmPassword: '' }} validationSchema={validationSchema} onSubmit={submitForm}>
                                {({ isSubmitting }) => (
                        ...
                        <p className="absolute bottom-6 w-full text-center dark:text-white">© {new Date().getFullYear()}Sentra Core Systems All Rights Reserved.</p>
                    </div>
                </div>
            </div>
        </div>
